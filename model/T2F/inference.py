...
def audio2head():
    ...
    parameters_file = parse.parameters

    '''여기부터 모델 선언'''
    from modules.audio2pose import audio2poseLSTM

    audio2poseLSTM = audio2poseLSTM().cuda()
    ckpt_para = torch.load(model_path)

    audio2poseLSTM.load_state_dict(ckpt_para["audio2pose"])
    audio2poseLSTM.eval()
    ...
    opt = argparse.Namespace(**yaml.load(open(parameters_file), Loader=yaml.FullLoader))
    ...
    temp_audio = "/home/user/PycharmProjects/IAI_chatbot/model/T2F/data/temp.wav"
    ...
    ref_pose_rot, ref_pose_trans = get_pose_from_audio(img, audio_feature, model_path, audio2poseLSTM)
    torch.cuda.empty_cache()

    audio_f = []
    poses = []
    pad = np.zeros((4, 41), dtype=np.float32)
    for i in range(0, frames, opt.seq_len // 2):  # opt.seq_len=64
        temp_audio = []
        temp_pos = []
