...
from decorators.decorators import data
from model.proc.base_processor import BaseProcessor
...
class Dataset:
    ...
    def load_embed(self) -> list:
        ...
        embed_dataset = pd.read_csv(self.intent_data_dir) # 생성된 인텐트 데이터셋 경로

        if self.ood:  # ood = True이면 ood도 로드해서 concat으로 합침
    ...
    def __map_label(self, dataset: DataFrame, kinds: str) -> list:
        ...
        # question: 사람의 발화, label : dust, restaurant, travel, weather
        if kinds == 'intent':
            labels = dataset['label'].map(self.intent_dict) # 각 label마다 숫자 부여
